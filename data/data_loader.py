...
class Dataset_Umass(Dataset):
    def __init__(self, target, flag='train', size=None, 
                 features='S', root_path=None, data_path=None, 
                 scale=True, inverse=False, timeenc=0, freq='h', useWeather=False):
    ...
    def _get_homeElectricalDir(root_path, home, year):
        return os.path.join(root_path, 'Home'+home+'-electrical', str(year))
    def _get_homeElectricalDataFileName(home, meter, year, filename_ext='.csv'):
        return 'Home' + home + '-meter' + str(meter) + '_' + str(year) + filename_ext
...
class Dataset_Custom(Dataset):
    ...
    # 和informer官方的数据集只在borders的设定那里有点不同
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
